  ...
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'POST') {
      ...
      try {
        ...
        } else if (url.pathname.startsWith('/update-task/')) {
          const id = url.pathname.split('/')[2];
          await updateTask(id, formData, env);
        } else if (url.pathname === '/add-category') {
          await addCategory(formData, env);
        } else if (url.pathname.startsWith('/update-category/')) {
          const id = url.pathname.split('/')[2];
      ...
      try {
        if (url.pathname.startsWith('/delete-task/')) {
          const id = url.pathname.split('/')[2];
          await deleteTask(id, env);
        } else if (url.pathname.startsWith('/delete-category/')) {
          const id = url.pathname.split('/')[2];
          await deleteCategory(id, env);
        } else if (url.pathname.startsWith('/reset-category/')) {
          const id = url.pathname.split('/')[2];
...
async function renderPage(view, categoryId, selectedDate, env) {
  ...
  let tasksRows = tasks
            ...
            <form id="update-form-${task.id}" method="post" action="/update-task/${task.id}" style="display:none;"></form>
        `;
                      })
                      .join('');
  ...
  let categoriesRows = categories
            ...
            <form id="update-category-form-${category.id}" method="post" action="/update-category/${category.id}" style="display:none;"></form>
        </tr>`;
                           })
                           .join('');
...
<head>
    ...
    function deleteCategory(id) {
        fetch('/delete-category/' + id, { method: 'DELETE' })
        .then(() => {
            location.reload();
        })
    ...
    function resetCategory(id) {
        fetch('/reset-category/' + id, { method: 'DELETE' })
        .then(() => {
            location.reload();
        })
...
<body>
    <div class="container mt-5">
        ...
        <div>
            <form action="/add-category" method="post" class="mb-4">
